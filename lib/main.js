...
function getIssuePanel() {
  if(!issuePanel) {
    ...
    issuePanel.port.on("search", function(data) {
      let searchTerm = data.search.replace(/[\t ]+/, " ").replace(" ", "+");
      ...
      getLabels(function(labels) {
        ...
        Request({
          ...
          onComplete: function(response) {
            ...
            let issues = prepareItems(response.json.issues).reverse();
...
function getPreferencesPanel() {
  if(!preferencePanel) {
    ...
    preferencePanel.port.on("submit", function(data) {
      for(let key in data) {
        prefs[key] = data[key];
      }
...
function getIssues(opts, callback) {
  ...
  let url = BASE_URL + "/issues" + getURLString(opts);
...
let Pages = {
  ...
  pageChange: function() {
    getIssuesCount(issueCountUpdate.bind(null, this.currPage));
...
function getPanelAnchor() {
  ...
  for(let index = 0, anc; index < len, anc = anchors[index]; index++) {
    if(anc.getAttribute("label") === config.name) {
      anchor = anc;
    }
  }
