...
issuePanel.port.on("search", function(data) {
  ...
  console.log("searching for: " + data.search + " : " + searchTerm);


  getLabels(function(labels) {
    var labelsHash = getLabelsHash(labels);
    var url = "http://github.com/api/v2/json/issues/search/" + config.repo_url + "/open/" + searchTerm;
    console.log(url);
    Request({
      url: url,
      onComplete: function(response) {
        // Issues by default come back in chronological order
        var issues = response.json.issues.reverse();
        console.log(issues);
